    ...
    * Aceita somente boletos com 44 digitos
    ...
    public static Double getValorBoleto(String codigo) {
        if (codigo.length() != 44) {
            ...
            return Double.parseDouble(codigo.substring(5, 15)) / 100;
        }
    }
    ...
    public String getVencimento(String codigo) {
        if (codigo.length() != 44) {
            throw new IllegalArgumentException("Tamanho do código de barras informado está incorreto!");
        } else {
            return CDate.SomarData(Integer.parseInt(codigo.substring(5, 9)), CDate.InicialBanco());
        }
    }
}
